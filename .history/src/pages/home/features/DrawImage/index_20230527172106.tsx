...
const DrawImage: FC<DrawImageProps> = (props) => {
  ...
  const handlerComputeRect = (
    ...
    // 获取子元素占据的比例是多少
    const { width: dirW, height: dirH } = position;

    const { width, height } = containerWH

    console.log(occupied.width, 'occupied.width');
    ...
    const handlerPositon = () => {
      ...
      if (direction === 'left') {
        if (flag === 'isEnd') {
          ...
          occupied.width = width * dirW - padding
          return {
            left,
            top: occupied.height,
            width: width * dirW - padding,
   ...
   * @returns ReactNode[]
   */
  const execute = (positions: LayoutConfig[], originWidth: number, originHeight: number) => {
        ...
        switch (direction) {
          case "left":
            elements = elements.concat(
              it.value?.length
                ? execute(it.value, Math.round(originWidth * it.width), Math.round(originHeight * it.height))
                : [
                    handlerComputeRect(
                      it,
                      occupied,
  ...
  console.log(execute(testLayoutCofig, containerWidth, containerHeight));
