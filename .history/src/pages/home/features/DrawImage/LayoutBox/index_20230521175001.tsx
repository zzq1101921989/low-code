...
const LayoutBox: FC<LayoutBoxProps> = (props) => {
    ...
    const handlerCreateElement = (config: LayoutConfig[], width: number, height: number): DOMElement<any, any>[] => {
        const ele = config.map(item => {
            if (item.value?.length) {
                ...
                }, handlerCreateElement(item.value, width * item.width, height * item.height))
            } else {
                return createElement('div', {
                    className: 'edit-box',
                    style: {
