...
const DrawImage: FC<DrawImageProps> = (props) => {
   ...
   * @param {number} config.left 左边距
   * @param {number} config.top 上边距
   * @param {number} config.width 宽度
   * @param {number} config.height 高度
   * @param {number} config.padding 间距
  ...
  const handlerComputeRect = (
    ...
    const { left, top, width, height, padding } = config;

    const { width: pW, height: pH } = position;
