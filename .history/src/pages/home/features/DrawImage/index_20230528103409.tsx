...
const DrawImage: FC<DrawImageProps> = (props) => {
  ...
  const execute = (
    ...
    positions.forEach((item, i) => {
      ...
      if (value?.length) {
        elements = elements.concat(execute(
          value,
          Math.round(originWidth * item.dirW),
          Math.round(originHeight * item.dirH)
        ))
      } else {

        // 第一个和最后一个都是需要特殊处理的
        const flag = isFirstOrLast(positions?.length, i);
  ...
  console.log(execute(testLayoutCofig, containerWidth, containerHeight), 'execute(testLayoutCofig, containerWidth, containerHeight)');
